...
// use a driver that sqlx knows uses a doller binding like postgres
...
// WrapTxnPopConnection sets up the pop Connection for this test
...
// tearDownTxnDbSuite closes the db connection for the txn test suite
func (suite *PopTestSuite) tearDownTxnDbSuite() {
	if suite.lowPrivConn == nil {
		return
	}
	err := suite.lowPrivConn.Close()
